...
<section class="bg-plain">
    <div class="container">
        <h1 class="m-b-xxs">Terms Of Use</h1>
...
<section class="bg-gradient-primary pad-t-m">
    <div class="container">
		...
		<?php 
			$termContent = "";
				if(empty($_GET)) {
					$url ="http://enablereview.biz/bmrnpolicy/?domain=achondroplasia2020.enablereview.biz&country=en-gb&cat=Term&terms_page=http://achondroplasia-expert.enablestaging.biz/privacy-policy/";
					$ch = curl_init();
					...
					if($_REQUEST['country_abbri'] =='en-GB1' || $_REQUEST['country_abbri'] =='en-GB2') {
						...
						$url = "http://enablereview.biz/bmrnpolicy/?domain=achondroplasia2020.enablereview.biz&country=".$_REQUEST['country_abbri']."&cat=Term&terms_page=http://achondroplasia-expert.enablestaging.biz/privacy-policy/";
					}			
...
?>
					...
					?>
					<strong>See below for BioMarin’s Terms of Use.  For other region/country versions, please select from the dropdown menu.</strong>
						<?php } else {?>
						<strong>See below for BioMarin’s Terms of Use.  For other region/country versions, please select from the dropdown menu.</strong>
					<?php } ?>
            <form name="form1" id="termForm" method="get" action="<?php //echo $_SERVER['PHP_SELF']; ?>">

								<select name="country_abbri" id="termPrivacyCountry1" style="max-width: 310px;">

									<option value="" selected="selected">Country Name</option>
                    <option value="au" <?php if(!empty($_REQUEST['country_abbri']) && $_REQUEST['country_abbri'] == 'au')?>>Australia (English)</option>
                    <option value="pt-BR" <?php if(!empty($_REQUEST['country_abbri']) && $_REQUEST['country_abbri'] == 'pt-BR') { echo "selected='selected'";}?>>Brazil (Português)</option>	
                    ...
                    <option value="ja-JP" <?php if(!empty($_REQUEST['country_abbri']) && $_REQUEST['country_abbri'] == 'ja-JP') { echo "selected='selected'";}?>>Japan (日本)</option>
